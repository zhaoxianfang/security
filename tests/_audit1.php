...
echo "══════ [1] 正则语法验证 ══════\n";
$errs = [];
foreach ($highRisk as $t=>$ps) foreach($ps as $p) {
    $pat = extractPat($p); if($pat && @preg_match($pat,'')===false) $errs[]="high_risk:$t:$pat";
}
foreach ($urlPath as $p) {
    $pat = extractPat($p); if($pat && @preg_match($pat,'')===false) $errs[]="url_path:$pat";
}
foreach ($xss as $t=>$ps) foreach($ps as $p) {
    $pat = extractPat($p); if($pat && @preg_match($pat,'')===false) $errs[]="xss:$t:$pat";
}
...
foreach($tests as $type=>$payloads){
  ...
  $hit=0;foreach($payloads as $pld)foreach($ps as $pt){ $pat=extractPat($pt); if($pat && @preg_match($pat,$pld)===1){$hit++;break;} }
...
foreach($urlT as list($pl,$d)){$h=false;foreach($urlPath as $pt){ $pat=extractPat($pt); if($pat && @preg_match($pat,$pl)===1){$h=true;break;} } echo($h?"✅":"❌")." $d\n";if($h)$p++;}
echo "  {$p}/".count($urlT)."\n";
...
$keys=['enabled','log_enabled','log_level','log_full_request',
  ...
  'rate_limit.*','intercept_rules','intercept_rules_exclude',
  'upload.*','max_url_length','max_body_size','user_agent_blacklist',
  'headers.*','headers.host_validation',
  'input_processing.*','response.*','response.view','markdown.*','encoding_detection.*',
  'allowed_http_methods','before_block_callback','excluded_routes','threat_risk_levels',
  'encoding_detection.suspicious_patterns',
];
$checks=[];
$dl=$config['detection_layers']??[]; $ul=$config['upload']??[]; $rl=$config['rate_limit']??[]; $hd=$config['headers']??[];
$ip=$config['input_processing']??[]; $rs=$config['response']??[]; $md=$config['markdown']??[]; $ed=$config['encoding_detection']??[];
...
$checks['intercept_rules']=is_array($config['intercept_rules']);$checks['intercept_rules_exclude']=is_array($config['intercept_rules_exclude']);
$checks['upload.*']=isset($ul['max_size']);
$checks['max_url_length']=isset($config['max_url_length']);$checks['max_body_size']=isset($config['max_body_size']);
$checks['user_agent_blacklist']=array_key_exists('user_agent_blacklist',$config);
$checks['headers.*']=is_array($hd['forbidden']??null)&&isset($hd['detect_crlf']);$checks['headers.host_validation']=isset($hd['host_validation']);
$checks['input_processing.*']=isset($ip['max_input_length']);$checks['response.*']=isset($rs['blocked_status']);
$checks['response.view']=array_key_exists('view',$rs);$checks['markdown.*']=isset($md['smart_detection'],$md['allow_script_in_markdown']);
$checks['encoding_detection.*']=isset($ed['percent_threshold']);
$checks['allowed_http_methods']=array_key_exists('allowed_http_methods',$config);
...
$checks['encoding_detection.suspicious_patterns']=array_key_exists('suspicious_patterns',$ed);
