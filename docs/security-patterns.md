...
检测服务器端请求伪造攻击，防止攻击者利用服务器发起对内网服务的请求：

| 攻击类型 | 示例 | 检测结果 |
|----------|------|----------|
| 内网IP访问 | `?url=http://192.168.1.1/admin` | 拦截 |
| 云元数据 | `?url=http://169.254.169.254/latest/meta-data/` | 拦截 |
...
| 攻击类型 | 示例 | 检测结果 |
|----------|------|----------|
| CRLF注入 | `?redirect=/page%0d%0aSet-Cookie:...` | 拦截 |
| 响应拆分 | 头部值包含 `\r\nContent-Type:...` | 拦截 |

中间件在 `hasInvalidHeaders()` 中自动检测所有请求头值是否包含 CRLF 字符（`\r` 或 `\n`），有效防止HTTP响应拆分攻击。
