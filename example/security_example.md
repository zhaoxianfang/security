...
return [
    ...
    // 使用自定义服务类
    'ip_whitelist' => [CustomSecurityService::class, 'getWhitelistIps'],
    ...
    'ip_whitelist' => function() {
        ...
        // 从环境变量读取
        $envIps = env('SECURITY_WHITELIST_IPS', '');
        if (!empty($envIps)) {
            $ips = array_merge($ips, explode(',', $envIps));
        }

        // 从数据库读取（如果可用）
        try {
            if (class_exists('App\Models\WhitelistIp')) {
                $dbIps = \App\Models\WhitelistIp::active()->pluck('ip_address')->toArray();
                $ips = array_merge($ips, $dbIps);
            }
        } catch (\Exception $e) {
            // 忽略数据库错误
        }
    ...
    'ip_whitelist' => function() {
        return cache()->remember('security:whitelist_ips', 300, function() {
            return [SecurityConfig::class, 'getWhitelistIps'];
        });
    },
    ...
    // 从威胁情报源获取
    'ip_blacklist' => [CustomSecurityService::class, 'getThreatIntelligenceIps'],
    ...
    'ip_blacklist' => function() {
        ...
        // 从文件读取
        $filePath = storage_path('security/blacklist_ips.txt');
        if (file_exists($filePath)) {
            $fileIps = file($filePath, FILE_IGNORE_NEW_LINES | FILE_SKIP_EMPTY_LINES);
            $blacklist = array_merge($blacklist, $fileIps);
        }

        // 从Redis获取动态黑名单
        if (class_exists('Illuminate\Support\Facades\Redis')) {
            $redis = Illuminate\Support\Facades\Redis::connection('security');
            $redisIps = $redis->smembers('security:blacklist_ips');
            $blacklist = array_merge($blacklist, $redisIps);
        }
